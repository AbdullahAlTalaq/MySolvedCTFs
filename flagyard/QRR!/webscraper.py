...
url = "https://blog.qr4.nl/Online-QR-Code-Decoder.aspx"
...
for value in values_list:
    ...
    data = {
        "ctl00$cphBody$TextBox1": value,
        "ctl00$cphBody$Button1": "Decode",
    }
    ...
    result_textarea = soup.find("textarea", {"id": "ctl00_cphBody_TextBox2"})

    if result_textarea:
        result = result_textarea.text.strip()
        print(f"Value: {value}\nResult: {result}\n")
    else:
        print(f"Failed to extract result for value: {value}\n")
session.close()